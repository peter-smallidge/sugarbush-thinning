...
#
# the source of this file is likely
# :\Users\pjs23\Documents\Projects\Maple\Maple research\Maple Response to Thinning\
#          Lumbershed Area\Crown Release Yield Per Acre\canister data....and the worksheet
#         "all.attribute"



```{r}
crown_lumbershed <- read_excel("lumbershed.static.2020_updated.xlsx")

crown_lumbershed <- crown_lumbershed %>% 
  rename(pct.open.2017 = ftg2017) %>% 
  select(seq, tag, crownclass2018, crownclass2020, strata.pre, strata.post,
         pct.open.2017, pct.open.jan.2019, FTG2019, crown.area.2017, numb.taps2017) %>% 
  mutate(location = "lumbershed") %>% 
  rename(ftg_post = FTG2019,
         strata_pre = strata.pre,
         strata_post = strata.post
         )

crown_decker <- read_excel("decker_static.xlsx", skip=3,
  ...
  #
  #
  #as of 12nov2021, these two data files need to merge by site, tag, etc
...
raw2021 <- X2021_sap_sugar_data %>% 
  mutate(
    sap_vol = volume * 0.231412,
    syrup_vol = (sap_vol / (87.5 / ssc))
    )

summ2021 <- raw2021 %>% 
  
#as of 12nov2021 need to acquire dbh data for both sites
# and calculate basal area and basal area increment
...
annual <- diam %>% 
  ...
  mutate(numb = 1) %>% 
  mutate(site = "lumbershed")
  select(seq, tag, year, numb, everything())

annual <- annual %>% 
  full_join(xxxx, by = c("site", "seq", "tag", "year"))
...
col01 <- annual %>% 
  ...
  labs(
    ...
    y = "Crown Area (sq. ft)",
    title = "Crown Area Among Treatments, col01, tidy-sap-syrup"
  )
...
col02 <- annual %>% 
  ...
  labs(
    ...
    title = "Average Growth Pre-Harvest (2017 & 2018), tidy-sap-syrup"
  )
...
col03 <- annual %>% 
  ...
  labs(
    ...
    y = "Basal Area Increment (sq inches)",
    title = "Average Growth Response Post-Harvest (2019 & 2020), tidy-sap-syrup"
  )
...
# set_palette(colq, "uchicago")
 ...
 col05 <-  annual %>%
  ...
  labs(
    ...
    y = "Tree Vigor (rBAI as % of 2017 Basal Area)",
    title = "Tree Vigor Response to Crown Class and Thinning",
    subtitle = "Harvest completed August 2018, col05, tidy-sap-syrup"
  )
...
year_sort <- year_adjusted %>% 
  filter(year_sap > 2017)
  ggplot((data = year_sort), 
       mapping = aes(x=crown.area.2017, y = bai)) +
  geom_point((mapping = aes(size = 3, color = strata.post))) +
  geom_smooth(method = lm, se = FALSE) +
  stat_cor(label.x.npc="left",label.y.npc = "top", 
           method="pearson", r.digits=2, p.digits=4, p.accuracy = 0.001)+
  stat_regline_equation(label.x.npc="center",label.y.npc = "left")+  
          #stat_cor and stat_regline within library(ggpubr)
  theme_classic()+
  ylim(0,10)+
  xlim(0,1000)+
...
#### Production Per Year by FTG

```{r}
#   mutate(year = as.character(year_sap)) %>% 
#       n = sum(is.na (numb)),
#   geom_col(aes(x=FTG2019, y=crown.area, fill = strata.pre), color = "black", position = "dodge2") +
...
x1 <-  year_adjusted %>% 
  select(year_sap, FTG2019, sap_vol, ssc, syrup_vol) %>% 
  mutate(FTG2019 = as.character(FTG2019)) %>% 
  arrange(year_sap) %>% 
  group_by(FTG2019, year_sap) %>% 
  filter(year_sap > 2016) %>% 
  summarize(
    sap_vol_mean = mean((sap_vol)),
    ssc_mean = mean((ssc)),
    syrup_vol_mean = mean((syrup_vol))
  )

head(x1, n=15)

x1 %>% 
  write_csv(path = "year_by_FTG.csv")
...
col20 <- x1 %>% 
  filter(year_sap >2017) %>% 
  ggplot((data=x1), 
         mapping = aes(x = year_sap, y = sap_vol_mean, color = FTG2019))+
  geom_col(aes(x=year_sap, y=sap_vol_mean, fill=FTG2019),color= "black", position= "dodge")+
      theme_classic()+
  #xlim(0,10)+
  labs(
    x = "Year",
    y = "Sap Volume (gal)",
    title = " Annual Sap Production by Free-To-Grow",
    subtitle = "Arnot Forest, harvested 8/2018, col20, tidy-sap-syrup"
  )
set_palette(col20, "uchicago")
  ggsave("figures/year-ftg-sap-vol.jpg")

  
col21 <- x1 %>% 
  filter(year_sap >2017) %>% 
  ggplot((data=x1), 
         mapping = aes(x = year_sap, y = syrup_vol_mean, color = FTG2019))+
  geom_col(aes(x=year_sap, y=syrup_vol_mean, fill=FTG2019),color= "black", position= "dodge")+
      theme_classic()+
  #xlim(0,10)+
  labs(
    x = "Year",
    y = "Syrup Volume (gal)",
    title = " Annual Syrup Production by Free-To-Grow",
    subtitle = "Arnot Forest, harvested 8/2018 col21, tidy-sap-syrup"
  )
set_palette(col21, "uchicago")

  ggsave("figures/year-ftg-syrup-vol.jpg")  
...
col22 <- x1 %>% 
  filter(year_sap >2017) %>% 
  ggplot((data=x1), 
         mapping = aes(x = year_sap, y = ssc_mean, color = FTG2019))+
  geom_col(aes(x=year_sap, y=ssc_mean, fill=FTG2019),color= "black", position= "dodge")+
      theme_classic()+
  #xlim(0,10)+
  labs(
    x = "Year",
    y = "Sap Volume (gal)",
    title = " Annual Sap Sugar Concentration by Free-To-Grow",
    subtitle = "Arnot Forest, harvested 8/2018,col22, tidy-sap-syrup"
  )
set_palette(col22, "uchicago")

  ggsave("figures/year-ftg-ssc-vol.jpg")  
```




### Tree Growth Response to 2020 Drought

```{r}


x2 <-  annual %>% 
  select(year, FTG2019, dbh, basal_area, bai, rbai, strata.pre, strata.post, crown.area.2017) %>% 
  mutate(FTG2019 = as.character(FTG2019)) %>% 
  arrange(year) %>% 
  group_by(FTG2019, year) %>% 
  filter(year > 2016) %>% 
...
x1 %>% 
  write_csv(path = "growth_year_by_FTG.csv")


```

```{r}
#,set_palette(palette="jco") other good = GnBu, uchicago,Greens, RdYlGn
# col23 <-  x2 %>% 
# set_palette(col23, "uchicago")
...
filter(year > 2017) %>% 
  ggplot((data = x2),
    mapping = aes(x=year, y = bai, color = FTG2019)) +
    geom_col(aes(x=year, y = bai, fill = FTG2019), position = "dodge")+
    theme_classic()+
    #ylim(0,5)+
    labs(
      x = "Year of Growth",
      y = "Basal Area Increment - bai (sq. in.)",
      title = "Annual BAI by Free-To-Grow Rating",
      subtitle = "Arnot Forest, Harvested 8/2018, col23, tidy-sap-syrup"
    )
set_palette(col23, "uchicago")
  ggsave("figures/col-year-ftg-bai.jpg") 


col24 <-  x2 %>% 
filter(year > 2017) %>% 
  ggplot((data = x2),
    mapping = aes(x=year, y = rbai, color = FTG2019)) +
    geom_col(aes(x=year, y = rbai, fill = FTG2019),  position = "dodge")+
    theme_classic()+
    #ylim(0,5)+
    labs(
      x = "Year of Growth",
      y = "Relative Basal Area Increment - rbai (% of 2017 BA)",
      title = "Annual rBAI by Free-To-Grow Rating",
      subtitle1 = "Arnot Forest, Harvested 8/2018, col24, tidy-sap-syrup"
    )
set_palette(col24, "uchicago")
  ggsave("figures/col-year-ftg-rbai.jpg") 

col25 <-  x2 %>% 
filter(year > 2017) %>% 
  ggplot((data = x2),
    mapping = aes(x=year, y = basal, color = FTG2019)) +
    geom_col(aes(x=year, y = basal, fill = FTG2019),  position = "dodge")+
    theme_classic()+
    #ylim(0,5)+
    labs(
      x = "Year of Growth",
      y = "Basal Area (sq. in.)",
      title = "Annual Basal Area by Free-To-Grow Rating",
      subtitle = "Arnot Forest, Harvested 8/2018, col25, tidy-sap-syrup"
    )
set_palette(col25, "uchicago")
  ggsave("figures/col-year-ftg-basal.jpg") 
...
filter(year > 2017) %>% 
  ggplot((data = x2),
    mapping = aes(x=year, y = dbh, color = FTG2019)) +
    geom_col(aes(x=year, y = dbh, fill = FTG2019),  position = "dodge")+
    theme_classic()+
    #ylim(0,5)+
    labs(
      x = "Year of Growth",
      y = "Medial DBH (in.)",
      title = "Annual DBH by Free-To-Grow Rating",
      subtitle = "Arnot Forest, Harvested 8/2018, col26, tidy-sap-syrup"
    )
set_palette(col26, "uchicago")
  ggsave("figures/col-year-ftg-dbh.jpg") 
